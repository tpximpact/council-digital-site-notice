...
import { ProcessApplicationResponse } from "@/types";

export async function processApplication(
  application: ApiPlanningApplication,
  ...
  try {
    const existingApplication =
      await getApplicationByApplicationNumber(applicationNumber);
...
export async function processMultipleApplications(
  applications: ApiPlanningApplication[],
): Promise<ProcessApplicationResponse[]> {
  const results = [];
