...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            ...
            const emailData = await createEmailData(applicationNumber, feeling, comment, postcode);
            await sendEmail(emailData);

            await savefeedbackToGoogleSheet(req.body);

            res.status(200).json({ message: 'Email sent & google sheet saved successfully' });
        } catch (error) {
